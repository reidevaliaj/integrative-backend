...
Use MOLLIE_MODE=test during client testing. A live switch requires the publisher's verified Mollie profile and its live_ API key installed in the backend's private .env as MOLLIE_API_KEY with MOLLIE_MODE=live. No key belongs in Git or the frontend. Retired monthly-price settings are for interpreting historical monthly records only; all new checkouts use the catalogue plan amount.

Live checkout additionally checks Mollie's enabled payment methods for the actual amount, currency and payment sequence. Annual purchases require both first-payment and recurring methods; single issues and archives require one-off methods. Pending approval, empty method lists and Mollie API failures keep checkout unavailable without preventing library access. The backend enforces the same check before creating a customer or order. Results are cached for 60 seconds, scoped by credential fingerprint, mode, currency, amount and sequence, so Mollie approval is picked up on subsequent page loads without another deployment. Embedded plans in existing-subscription responses are informational; purchase availability is provided by the plans endpoint for the selected volume year.
